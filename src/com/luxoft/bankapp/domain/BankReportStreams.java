...
public class BankReportStreams {
    ...
    public double getTotalSumInAccounts(Bank bank) {
        return bank.getClients().stream().map(Client::getAccounts).flatMap(Collection::stream).mapToDouble(Account::getBalance).sum();
    ...
    public SortedSet<Account> getAccountsSortedBySum(Bank bank) {
        ...
        treeSet.addAll(bank.getClients().stream().map(Client::getAccounts).flatMap(Collection::stream).toList());
        return treeSet;
    }

    public double getBankCreditSum(Bank bank) {
        return bank.getClients().stream().map(Client::getAccounts).flatMap(Collection::stream).filter(CheckingAccount.class::isInstance).map(CheckingAccount.class::cast).mapToDouble(CheckingAccount::getOverdraft).sum();
